...
2. EXACTLY 8-9 bullet points, covering, in this order:
   a. *Core nature* — one bullet on their essential temperament, drawn from the Lagna lord's placement/status or a standout planet, cross-checked with the palm's overall hand shape and most dominant mount.
   ...
   f. *Strengths* — one bullet on their natural gifts and where they shine, drawing on supportive planets/yogas in the chart AND a well-formed mount or a strong, clear major line in the palm. Keep it specific to them, not generic praise.
   g. *Growth edges* — one bullet on the recurring weakness or blind spot to gently work on, framed kindly and constructively; you may note a palm cue (e.g. a wavy or broken major line) but strictly as a tendency, never a defect or a medical/lifespan claim.
   h. *Communication style* — one bullet on how they express themselves, listen, and connect with others, reading Mercury / the 3rd house in the chart alongside the palm's finger set, Mercury mount and Head line.
   i. *What's ahead* — one closing bullet, forward-looking: the next notable shift (an upcoming Antardasha change, an easing/intensifying transit) and a gentle, practical note on navigating the current period.

Throughout, treat the palm photo strictly as a character/temperament cue — never as a medical, identity, age, or lifespan claim — and never narrate the image clinically. Where the chart and the palm point the same way, say so naturally.
...
/**
 * Produce a detailed astro read (opening line + 8-9 bullets) from a precisely
 ...
 * @returns {Promise<string>} opening line + 8-9 bullet astro read
 */
...
Write the opening line plus the 8-9 point astro read connecting this chart and their exact birth details to their actual situation above. Write the entire response in ${outputLanguage}.`;
